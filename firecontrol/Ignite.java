...
public class Ignite extends SimState{
	...
	/* simulation params */
	public int numUAVs = 12; //number of mavs involved in the simulation
	public int totalAssigned;
	public int TasksCompletedQty;
	public boolean Remaining_Utility = false;
	public boolean Round2on = false;
	public boolean Remaining = false;
	public LinkedList<Integer> uavIDassigned = new LinkedList<Integer>();
	public LinkedList<Integer> uavIDremaining = new LinkedList<Integer>();
	// IntStream.range(0, numUAVs).collect(Collectors.toCollection(LinkedList::new));
	// public Bag UAVs; // all the agents in the simulation. Bag size is numMavs
	public LinkedList<UAV> UAVs;
	public LinkedList<Double> remainingTaskUtility = new LinkedList<Double>();
	public LinkedList<Integer> remainingtaskIDs = new LinkedList<Integer>();
	...
	int fires = 3;
	int NumManagersAssigned = 0;
	public LinkedList<Integer> ManagerIDs = new LinkedList<Integer>();
	public boolean ReselectTask; // used when a fire is extinguished and uavs need new task
	...
	public void start(){
		...
		// create list with uav ID left to assigned
		for (int i=0; i<numUAVs; i++){
			uavIDremaining.add(i);
		}
		...
		for(Task task : this.tasks){
			task.pickManager(this);
			System.err.println("Manager for task " + task.id + ": is UAV " + task.manager.id);
		}
